...
/** 확장자 → 언어. 임무·역할·팀 파일이 md/json이라 3종이면 실사용을 덮는다 */
...
// 구문 색 — 상태 색 관례를 따른다: 키·헤딩 = blue, 문자열 = green, 숫자·상수 = amber,
// 주석·메타 = dim. 팔레트를 새로 만들지 않는다 (토큰이 원본).
const eqHighlight = HighlightStyle.define([
  { tag: [t.keyword, t.propertyName, t.definitionKeyword], color: "var(--eq-blue)" },
  { tag: [t.heading], color: "var(--eq-blue)", fontWeight: "700" },
  { tag: [t.string, t.special(t.string)], color: "var(--eq-green)" },
  { tag: [t.number, t.bool, t.null, t.atom], color: "var(--eq-amber)" },
  { tag: [t.comment, t.meta, t.processingInstruction], color: "var(--eq-dim)" },
