    ...
    <div class="overlay" onClick={() => !busy() && setExitOpen(false)}>
      <div class="dialog" onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: "18px 20px 0" }}>
          <div style={{ "font-size": "16px", "font-weight": 800 }}>{t("EQMUX를 종료할까요?")}</div>
          <div class="muted" style={{ "font-size": "12px", "margin-top": "4px" }}>
            {tf("실행 중인 세션 {n}개가 종료됩니다. 자동 재개하지 않습니다.", { n: running().length })}
          </div>
        </div>
        <div class="card inset" style={{ margin: "14px 20px", padding: "8px 12px" }}>
          <span class="mono" classList={{ muted: !busy(), "st-waiting": busy() }} style={{ "font-size": "11px" }}>
            {busy() ? t(PHASE_TEXT[phase()!]) : t("입력 차단 → 저장소 flush → 정상 종료 신호 → 유예 후 트리 종료 (Job Object)")}
          </span>
        </div>
        <div style={{ padding: "0 20px", "max-height": "260px", "overflow-y": "auto" }}>
          <For each={running()}>
            {(s) => (
              <div class="kv">
                <span>
                  <b>{persona(s.personaId)?.name ?? t("기본 터미널")}</b>{" "}
                  <span class="muted">
                    {ws(s.workspaceId)?.name} · {s.status}
                  </span>
                </span>
                <span class="mono" classList={{ "st-dead": !s.resumable, "st-busy": s.resumable }}>
                  {s.resumable ? t("재개 가능") : `${t("재개 불가")} · ${t(s.resumeReason ?? "transcript 없음")}`}
                </span>
              </div>
            )}
          </For>
