      ...
      <div class="screen-head">
        ...
        <div style={{ display: "flex", gap: "8px", "align-items": "center", "flex-wrap": "wrap" }}>
          ...
          <Show
            when={addOpen()}
            fallback={
              <button class="btn primary" title="전역 라이브러리에 추가 — 단계를 고릅니다" onClick={() => setAddOpen(true)}>
                + 페르소나 추가
              </button>
            }
          >
            <span class="eyebrow">단계</span>
            <button class="btn" title="이름 · 색 · 판단 성향" onClick={() => void addWithLevel("basic")}>
              기본
            </button>
            <button class="btn" title="+ 말투 · 성격 옵션" onClick={() => void addWithLevel("mid")}>
              중급
            </button>
            <button
              class="btn"
              title="+ 캐릭터 시트 (personas/<id>.character.md)"
              disabled={!isTauri()}
                    ...
                    <div style={{ "min-width": 0 }}>
                      <div style={{ "font-weight": 700, display: "flex", gap: "6px", "align-items": "center" }}>
                        ...
                        <Show when={personaLevel(p) === "mid"}>
                          <span class="badge green" title="중급 — 말투·성격까지 주입됩니다">
                            중급
                          </span>
                        </Show>
                        <Show when={personaLevel(p) === "adv"}>
                          <span class="badge amber" title={`고급 단계 — 캐릭터 시트: ${p.characterPath ?? ""}`}>
                            고급
                          </span>
                        </Show>
                      </div>
              ...
              <>
                ...
                <div style={{ display: "flex", "flex-direction": "column", gap: "8px", "margin-top": "8px" }}>
                  ...
                  <div class="card inset" style={{ padding: "4px 10px" }}>
                    ...
                    <KV k="구성" v="8종 고정 — 문구·권한 편집만 가능, 지워도 부팅 시 부활" />
          ...
          <Show when={sel()}>
            ...
            <div style={{ display: "flex", "flex-direction": "column", gap: "8px", "margin-top": "8px" }}>
              ...
              <div style={{ display: "flex", gap: "6px" }}>
                ...
                >
                  {([lvl, label, desc]) => (
                    <button class="btn" classList={{ primary: editLevel() === lvl }} title={desc} onClick={() => setEditLevel(lvl)}>
                      {label}
                    </button>
                  )}
                </For>
              ...
              <Show when={editLevel() !== "adv"}>
                <label class="muted" style={{ "font-size": "11px" }}>
                  판단 성향 ({LEVEL_NAME[editLevel()]} 프로필) — 판단 우선순위 · 강조점 · 금기 (FR-E-25) · 예산 5~10줄 (P3)
                </label>
              ...
              <label class="muted" style={{ "font-size": "11px" }}>
                말투 ({LEVEL_NAME[editLevel()]} 프로필) — 응답의 어조. 비우면 지시하지 않음
              </label>
              <div style={{ display: "flex", gap: "6px", "flex-wrap": "wrap" }}>
                <For each={TONE_PRESETS}>
                  {(t) => (
                    <button
                      class="btn ghost"
                      classList={{ primary: prof().tone === t }}
                      style={{ "font-size": "11px" }}
                      onClick={() => setProf({ tone: prof().tone === t ? "" : t })}
                    >
                      {t}
                    </button>
                  )}
              ...
              <input
                placeholder="예: 간결한 존댓말 · 결론부터 말한다"
                value={prof().tone}
                onInput={(e) => setProf({ tone: e.currentTarget.value })}
              />
              <label class="muted" style={{ "font-size": "11px" }}>
                성격 ({LEVEL_NAME[editLevel()]} 프로필) — 행동 경향 1~{CHAR_BUDGET}줄. 판단과 겹치면 판단 성향에
              </label>
              <textarea
                rows={2}
                placeholder="예: 신중함 — 되돌릴 수 없는 결정을 늦춘다"
              ...
              <Show when={lineCount(prof().tone) > CHAR_BUDGET || lineCount(prof().personality) > CHAR_BUDGET}>
                <div class="mono st-waiting" style={{ "font-size": "11px" }}>
                  예산 — 말투·성격 각 {CHAR_BUDGET}줄 권장. 긴 캐릭터 묘사는 토큰만 먹습니다 (P3)
                </div>
              </Show>
              </Show>
                ...
                <Show
                  ...
                  fallback={
                    <button
                      class="btn"
                      disabled={selIsWs()}
                      title="personas/<id>.character.md 템플릿 생성 — 이름·출처만 채워도 에이전트가 원전 지식으로 보완합니다"
                ...
                >
                  <div class="card inset" style={{ padding: "8px 10px", display: "flex", "flex-direction": "column", gap: "6px" }}>
                    <div style={{ display: "flex", gap: "6px", "align-items": "center", "flex-wrap": "wrap" }}>
                      <span class="badge amber">고급</span>
                    ...
                    <div style={{ display: "flex", gap: "6px" }}>
                      ...
                      >
                        <button
                          class="btn primary"
                          disabled={selIsWs()}
                          title={selIsWs() ? "오버라이드 시트는 .eqmux/personas 파일을 직접 편집하세요" : "앱 내 편집 — 저장 시 캐스팅 세션에 알립니다"}
                          onClick={() => void openSheet(sel()!.id)}
                        >
                          시트 편집
                        </button>
                      </Show>
                      <button class="btn danger" disabled={selIsWs()} title="시트 파일 삭제 — 단계 설정(고급)은 유지됩니다" onClick={() => void demoteSheet()}>
                        시트 삭제
                      </button>
                    </div>
                  </div>
                  <Show when={sheetOpen()}>
                    ...
                    <button class="btn primary" onClick={() => void saveSheet()}>
                      {sheetSaved() ? "시트 저장됨 ✓ — 캐스팅 세션에 알림" : "시트 저장"}
                    </button>
                  </Show>
                </Show>
              </Show>
              <div class="card inset" style={{ padding: "4px 10px" }}>
                ...
                <KV k="주입" v=".eqmux/roles/<세션>.md 합성 — 시트는 경로 포인터만" />
              </div>
              <Show when={selIsWs()}>
                <div class="mono st-waiting" style={{ "font-size": "10px" }}>
