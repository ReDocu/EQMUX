...
function fmtLastActivity(ms: number | null): string {
  ...
  const rel = min < 1 ? "방금" : min < 60 ? `${min}분 전` : min < 1440 ? `${Math.floor(min / 60)}시간 전` : `${Math.floor(min / 1440)}일 전`;
  return `${abs} (${rel})`;
}
    ...
    <div class="overlay" onClick={props.onClose}>
      <div class="dialog" style={{ width: "560px" }} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: "18px 20px 0" }}>
          <div style={{ "font-size": "16px", "font-weight": 800 }}>직전 실행이 정상 종료되지 않았습니다</div>
