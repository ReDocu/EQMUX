...
**하나의 git 저장소를 AI 에이전트 팀이 함께 작업하고, 사람이 그것을 관제하는 데스크톱 앱.**

EQMUX는 터미널 멀티플렉서(MUX)에 에이전트 팀 관제를 얹은 Windows 데스크톱 앱입니다.
워크스페이스(= git repo 1개 = 팀 1개 = 탭 1개)마다 Claude Code 세션을 기본 4개(설정에서 6·8까지)
병렬로 띄우고, 관제 대시보드에서 상태를 보고, 임무를 배정하고, 필요할 때 개입합니다.
...
- **터미널 페인이 작업 공간이다.** 실제 일은 각 페인의 에이전트 CLI(Claude Code)가 한다.
...
세션 슬롯 N은 기본 4개이고 설정에서 6·8로 올릴 수 있습니다. 기본값이 4인 것은 리소스가 아니라
**가독성** 때문입니다 — 1920×1080 기준 2×2 분할이 에이전트 TUI를 읽을 수 있는 한계입니다.
팀 편성(`team.json`)은 이 설정과 무관하게 최대 8슬롯을 허용합니다.
...
**워크스페이스** — 터미널 분할이 작업 공간입니다(스크린샷은 기본 4분할). 왼쪽에 세션·임무 목록, 오른쪽 인스펙터에서
...
**팀 캐스팅** — 프리셋(표준 / 집중구현 / 리뷰중심 / 탐색)으로 각 슬롯에 직무 + 페르소나를
