...
**1차 (터미널 모드) · S1 진행 중.** 날짜 없이 관문 통과로만 진행한다.

| 단계 | 목표 | 상태 |
|---|---|---|
| S0 | 준비 — 기준선·전제·스파이크·저장소 | **5/6** — `S0-2` 기능 대조표만 남음 |
| S1 | Walking Skeleton → **관문 A** | **구현 완료** — S1-1·S1-2·S1-3·S1-4 ✅ / **관문 A 검증 대기** |
...
>
> ⏸️ **관문 A는 아직 보류다** ([docs/GATE-A.md](docs/GATE-A.md)).
> A-1(IME) ✅ · **A-3(키 입력 지연) ✅ 통과** — 실작업 p99 **0.4 ms**(≤8) · 총지연 p99 **16.9 ms**(≤33.4).
> `S1-3b` 4점 분리로 잴 수 있게 됐고, **계측기는 자가 검증 3항을 통과해 인수됐다**
> ([docs/LATENCY-S1-3b.md](docs/LATENCY-S1-3b.md) §10).
>
> ⚠️ **A-2(CJK 폭)는 계측 통과 · 육안 대기** ([docs/FONT-A2.md](docs/FONT-A2.md) §0-A).
...
> 굵게까지 **오차 0.000px**다. **남은 것은 육안 판정 하나다.**
